...
static int *
uint32ToUnit8Array(uint32_t value)
{
  static int arr[4];
  arr[0] = (value >> 24) & 0xFF;
  arr[1] = (value >> 16) & 0xFF;
...
static void
randomBytes(uint8_t *buffer, int length)
{
...
static void
uint32ToBytes(uint32_t value, uint8_t *bytes)
{
    ...
    const uint8_t *channel_key)
{
  ...
  uint32_t packet_id = ((uint32_t)packet_id_bytes[0] << 24) |
                       ...
                       ((uint32_t)packet_id_bytes[2] << 8) |
                       (uint32_t)packet_id_bytes[3];
  ...
  if (data_obj->payload && data_obj->payload_len > 0)
  {
    ...
    if (data_obj->payload_len > 0)
    {
      ...
      protobuf_buffer[protobuf_len++] =
          data_obj->payload_len; // assume length < 128
      memcpy(
          &protobuf_buffer[protobuf_len], data_obj->payload,
          data_obj->payload_len);
    ...
    for (int i = 0;i< protobuf_len;i++) {
        printf("%02X ", output_buffer[packet_len + i]);
    }
    printf("\n");
...
int
hexStringToBytes(const char *hex_str, uint8_t *output, int max_len)
{
...
int
readVarint(const uint8_t *data, int pos, int max_pos, uint32_t *value)
{
  if (pos >= max_pos)
    return -1;
...
int
decodeProtobufData(
    const uint8_t *data, int data_len, DecodedDataMessage *decoded)
{
  ...
  while (pos < data_len)
  {
    ...
    { // varint
      ...
      case 1:
        ...
        printf(
            " (portnum: %s)", value == 1   ? "TEXT_MESSAGE"
                              : value == 3 ? "POSITION"
                              : value == 4 ? "NODEINFO"
                                           : "UNKNOWN");
        break;
...
void
decodeProtobufFromHex(const char *hex_str)
{
  ...
  {
    printf(
        "❌ Invalid packet: too short (need at least 16 bytes for header)\n");
    return;
  }
  ...
  if (packet_len > 16)
  {
    ...
    int result =
        decodeProtobufData(&packet_data[16], packet_len - 16, &decoded);
